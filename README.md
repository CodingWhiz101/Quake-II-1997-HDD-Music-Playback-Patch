...
| File | Purpose |
| --- | --- |
| [Q2Win95Prep/](Q2Win95Prep/) | Modern GUI preparer (`build.cmd` → `Q2Win95Prep.exe`) |
| [WIN95_README.TXT](WIN95_README.TXT) | In-folder sheet; the preparer regenerates this |
| [SETUP95.BAT](SETUP95.BAT) | Optional: copy this PC’s mixer to `winmmsys.dll` before launch |
| [VERIFY95.BAT](VERIFY95.BAT) | Layout check (shim, WAVs, PAKs, no GOG leftovers) |
| [XATRIX.BAT](XATRIX.BAT) / [ROGUE.BAT](ROGUE.BAT) | Expansion launches |
| [_analysis/q2cd_winmm.c](_analysis/q2cd_winmm.c) | Shim source (Watcom; not needed by preparer users) |
| [_analysis/build.cmd](_analysis/build.cmd) | Open Watcom rebuild of `wincd.dll` |
| [_analysis/prepare_pcm_tracks.py](_analysis/prepare_pcm_tracks.py) | Optional FFmpeg conversion (preparer uses stb_vorbis) |
...
That embeds the already-built `..\wincd.dll`. Rebuild the shim with
`_analysis\build.cmd` only if you changed `q2cd_winmm.c`.
